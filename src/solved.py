...
def Problem1():
    ...
    """
    answers_li = []
    for ea in range(1000):
        if ea%3 == 0:
            answers_li.append(ea)
        elif ea%5 == 0:
            answers_li.append(ea)
    return sum(answers_li)
...
def Problem3():
    ...
    """
    return max(util.prime_factors(600851475143))
...
def Problem8(num=, consecutive=13):
    ...
    for i in range(1,len(li)-consecutive):
        subset_mult = util.multiply_list(li[i:i+consecutive])
        if subset_mult > highest:
            highest = subset_mult
    return highest